...
class RolMenuController extends Controller {
    ...
    public function ifcanIndex($action, &$nivel, $model = null) {
        ...
        $puede = Vpermisos::findBySql(
                        ...
                        . ' and ( permi_metodo = :action or permi_metodo = ' . "'*'" . ' )', ['clase' => 'RolMenu',
                    'usr' => $usr,
                    'action' => $action])->all();
        $r = false;
        if (sizeof($puede) > 0) {
...
//$dataProvider->query->andFilterWhere(['event_id' => $id,]);
    ...
    public function actionCreate() {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save()) {
                return $this->redirect(['view', 'id' => $model->rolme_id]);
    ...
    /**
     * crea en batch nuevosRolMenu modelos hijos de una table.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionAddBatch($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->save()) {
                return $this->redirect(['RolMenu/add', 'regis' => $ri]);
            } else {
    ...
    public function actionAdd($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            $model->validate();
            if ($model->save()) {
                return $this->redirect(['roles/view', 'id' => $model->rol_id]);
            } else
                return print_r($model);
        } else {
            $model->rol_id = $idMaster;
    ...
    public function actionDelete($id) {
         ...
         */
        $rol_id = $model->rol_id;
        return $this->redirect(['roles/view', 'id' => $rol_id]);
        ;
    }
    ...
    protected function findModel($id) {
        if (($model = RolMenu::findOne($id)) !== null) {
            return $model;
        }

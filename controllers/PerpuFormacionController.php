...
/**
 * PerpuFormacionController implements the CRUD actions for PerpuFormacion model.
 */
class PerpuFormacionController extends Controller {
    ...
    public function ifcanIndex($action, &$nivel, $model = null) {
        ...
        $puede = Vpermisos::findBySql(
                        ...
                        . ' and ( permi_metodo = :action or permi_metodo = ' . "'*'" . ')', ['clase' => 'PerpuFormacion',
                    'usr' => $usr,
                    'action' => $action])->all();
        $r = false;
        if (sizeof($puede) > 0) {
...
//$dataProvider->query->andFilterWhere(['event_id' => $id,]);
    ...
    /**
     * Lists all PerpuFormacion models.
     * @return mixed
     */
    ...
    /**
     * Displays a single PerpuFormacion model.
     * @param integer $id
     * @return mixed
    ...
    /**
     * Creates a new PerpuFormacion model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreate() {
        $this->can('create');
        $model = new PerpuFormacion();
    ...
    /**
     * crea en batch nuevosPerpuFormacion modelos hijos de una table.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionAddBatch($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->save()) {
                return $this->redirect(['PerpuFormacion/add', 'regis' => $ri]);
            } else {
    ...
    /**
     * Updates an existing PerpuFormacion model.
    ...
    /**
     * Deletes an existing PerpuFormacion model.
    ...
    /**
     * Finds the PerpuFormacion model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return PerpuFormacion the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id) {
        if (($model = PerpuFormacion::findOne($id)) !== null) {
            return $model;
        }

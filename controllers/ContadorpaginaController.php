...
use app\models\ContadorpaginaSearch;
...
/**
* ContadorpaginaController implements the CRUD actions for Contadorpagina model.
*/
class ContadorpaginaController extends Controller
{
...
public   function ifcanIndex($action, &$nivel, $model = null) {
    ...
    $puede = Vpermisos::findBySql(
                    ...
                    . ' and ( permi_metodo = :action or permi_metodo = '. "'*'".')', ['clase' => 'Contadorpagina',
                'usr' => $usr,
                'action' => $action])->all();
    $r = false;
    if (sizeof($puede) > 0) {
...
/**
* Lists all Contadorpagina models.
* @return mixed
*/
public function actionIndex()
{
    $this->canIndex('index', $nivel);
    $searchModel = new ContadorpaginaSearch();
...
/**
* Displays a single Contadorpagina model.
* @param integer $id
* @return mixed
...
/**
* Creates a new Contadorpagina model.
...
{
    $this->can('create');
    $model = new Contadorpagina();
    if ($model->load(Yii::$app->request->post())) {
        if ($model->save()) {
            return $this->redirect(['view', 'id' => $model->conpa_id]);}
        else
...
/**
* crea en batch nuevosContadorpagina modelos hijos de una table.
* If creation is successful, the browser will be redirected to the 'view' page.
* @return mixed
*/
public function actionAddBatch($idMaster) {
    ...
    if ($model->load(Yii::$app->request->post())) {
        ...
        if ($model->save()) {
            return $this->redirect(['Contadorpagina/add', 'regis' => $ri]);
        } else {
...
public function actionAdd($idMaster)
{
    ...
    if ($model->load(Yii::$app->request->post()) ) {
        if ($model->save())
        return $this->redirect(['Contadorpagina/view', 'id' => $model->perca_id]);
         }
    else {
...
/**
* Updates an existing Contadorpagina model.
...
/**
* Deletes an existing Contadorpagina model.
...
/**
* Finds the Contadorpagina model based on its primary key value.
* If the model is not found, a 404 HTTP exception will be thrown.
* @param integer $id
* @return Contadorpagina the loaded model
* @throws NotFoundHttpException if the model cannot be found
*/
protected function findModel($id)
{
if (($model = Contadorpagina::findOne($id)) !== null) {
    return $model;
    }

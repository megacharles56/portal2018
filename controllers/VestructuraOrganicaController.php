...
/**
* VestructuraOrganicaController implements the CRUD actions for VestructuraOrganica model.
*/
class VestructuraOrganicaController extends Controller
{
...
public   function ifcanIndex($action, &$nivel, $model = null) {
    ...
    $puede = Vpermisos::findBySql(
                    ...
                    . ' and ( permi_metodo = :action or permi_metodo = "*")', ['clase' => 'VestructuraOrganica',
                'usr' => $usr,
                'action' => $action])->all();
    $r = false;
    if (sizeof($puede) > 0) {
...
/**
* Lists all VestructuraOrganica models.
* @return mixed
*/
...
/**
* Displays a single VestructuraOrganica model.
* @param integer $id
* @return mixed
...
/**
* Creates a new VestructuraOrganica model.
...
/**
* crea en batch nuevosVestructuraOrganica modelos hijos de una table.
* If creation is successful, the browser will be redirected to the 'view' page.
* @return mixed
*/
public function actionAddBatch($idMaster) {
    ...
    if ($model->load(Yii::$app->request->post())) {
        ...
        if ($model->save()) {
            return $this->redirect(['VestructuraOrganica/add', 'regis' => $ri]);
        } else {
...
public function actionAdd($idMaster)
{
    ...
    if ($model->load(Yii::$app->request->post()) ) {
        if ($model->save())
        return $this->redirect(['VestructuraOrganica/view', 'id' => $model->perca_id]);
         }
    else {
...
/**
* Updates an existing VestructuraOrganica model.
...
/**
* Deletes an existing VestructuraOrganica model.
...
/**
* Finds the VestructuraOrganica model based on its primary key value.
* If the model is not found, a 404 HTTP exception will be thrown.
* @param integer $id
* @return VestructuraOrganica the loaded model
* @throws NotFoundHttpException if the model cannot be found
*/
protected function findModel($id)
{
if (($model = VestructuraOrganica::findOne($id)) !== null) {
    return $model;
    }

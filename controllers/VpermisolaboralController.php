...
/**
 * VpermisolaboralController implements the CRUD actions for Vpermisolaboral model.
 */
class VpermisolaboralController extends Controller {
    ...
    public function ifcanIndex($action, &$nivel, $model = null) {
        ...
        $puede = Vpermisos::findBySql(
                        ...
                        . ' and ( permi_metodo = :action or permi_metodo = ' . "'*'" . ')', ['clase' => 'Vpermisolaboral',
                    'usr' => $usr,
                    'action' => $action])->all();
        $r = false;
        if (sizeof($puede) > 0) {
...
//$dataProvider->query->andFilterWhere(['event_id' => $id,]);
    ...
    /**
     * Lists all Vpermisolaboral models.
     * @return mixed
     */
    ...
    /**
     * Displays a single Vpermisolaboral model.
     * @param integer $id
     * @return mixed
    ...
    /**
     * Creates a new Vpermisolaboral model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreate() {
        $this->can('create');
        $model = new Vpermisolaboral();
    ...
    /**
     * crea en batch nuevosVpermisolaboral modelos hijos de una table.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionAddBatch($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->save()) {
                return $this->redirect(['Vpermisolaboral/add', 'regis' => $ri]);
            } else {
    ...
    public function actionAdd($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save())
                return $this->redirect(['Vpermisolaboral/view', 'id' => $model->perca_id]);
        }
        else {
    ...
    /**
     * Updates an existing Vpermisolaboral model.
    ...
    /**
     * Deletes an existing Vpermisolaboral model.
    ...
    /**
     * Finds the Vpermisolaboral model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Vpermisolaboral the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id) {
        if (($model = Vpermisolaboral::find(['perla_Id' => $id])) !== null) {
            return $model;
        }

        if (($model = Vpermisolaboral::findOne($id)) !== null) {
            return $model;
        }

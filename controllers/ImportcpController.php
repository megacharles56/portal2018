...
/**
 * ImportcpController implements the CRUD actions for Importcp model.
 */
class ImportcpController extends Controller {
    ...
    public function ifcanIndex($action, &$nivel, $model = null) {
        ...
        $puede = Vpermisos::findBySql(
                        ...
                        . ' and ( permi_metodo = :action or permi_metodo = "*")', ['clase' => 'Importcp',
                    'usr' => $usr,
                    'action' => $action])->all();
        $r = false;
        if (sizeof($puede) > 0) {
...
//$dataProvider->query->andFilterWhere(['event_id' => $id,]);
    ...
    /**
     * Lists all Importcp models.
     * @return mixed
     */
    public function actionIndex() {
        $this->canIndex('index', $nivel);
        $searchModel = new ImportcpSearch();
    ...
    /**
     * Displays a single Importcp model.
     * @param integer $id
     * @return mixed
    ...
    public function actionCreate() {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save()) {
                return $this->redirect(['view', 'id' => $model->impcp_id]);
    ...
    /**
     * crea en batch nuevosImportcp modelos hijos de una table.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionAddBatch($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->save()) {
                return $this->redirect(['Importcp/add', 'regis' => $ri]);
            } else {
    ...
    public function actionAdd($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save())
                return $this->redirect(['Importcp/view', 'id' => $model->perca_id]);
        }
        else {
    ...
    /**
     * Updates an existing Importcp model.
    ...
    /**
     * Deletes an existing Importcp model.
    ...
    /**
     * Finds the Importcp model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Importcp the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id) {
        if (($model = Importcp::findOne($id)) !== null) {
            return $model;
        }

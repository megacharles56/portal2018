...
/**
 * SalonesLigadosController implements the CRUD actions for SalonesLigados model.
 */
class SalonesLigadosController extends Controller {
    ...
    public function ifcanIndex($action, &$nivel, $model = null) {
        ...
        $puede = Vpermisos::findBySql(
                        ...
                        . ' and ( permi_metodo = :action or permi_metodo = ' . "'*'" . ')', ['clase' => 'SalonesLigados',
                    'usr' => $usr,
                    'action' => $action])->all();
        $r = false;
        if (sizeof($puede) > 0) {
...
//$dataProvider->query->andFilterWhere(['event_id' => $id,]);
    ...
    /**
     * Lists all SalonesLigados models.
     * @return mixed
     */
    ...
    /**
     * Displays a single SalonesLigados model.
     * @param integer $id
     * @return mixed
    ...
    /**
     * Creates a new SalonesLigados model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreate() {
        $this->can('create');
        $model = new SalonesLigados();
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save()) {
                return $this->redirect(['view', 'id' => $model->salig_id]);
    ...
    /**
     * crea en batch nuevosSalonesLigados modelos hijos de una table.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionAddBatch($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->save()) {
                return $this->redirect(['SalonesLigados/add', 'regis' => $ri]);
            } else {
    ...
    public function actionAdd($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save())
                return $this->redirect(['salones/view', 'id' => $model->salon_id]);
        }
        else {
            $model->salon_id = $idMaster;
    ...
    /**
     * Updates an existing SalonesLigados model.
    ...
    /**
     * Deletes an existing SalonesLigados model.
    ...
    /**
     * Finds the SalonesLigados model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return SalonesLigados the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id) {
        if (($model = SalonesLigados::findOne($id)) !== null) {
            return $model;
        }

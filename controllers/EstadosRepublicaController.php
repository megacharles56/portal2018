...
use app\models\EstadosRepublicaSearch;
...
/**
 * EstadosRepublicaController implements the CRUD actions for EstadosRepublica model.
 */
class EstadosRepublicaController extends Controller {
    ...
    public function ifcanIndex($action, &$nivel, $model = null) {
        ...
        $puede = Vpermisos::findBySql(
                        ...
                        . ' and ( permi_metodo = :action or permi_metodo = ' . "'*'" . ')', ['clase' => 'EstadosRepublica',
                    'usr' => $usr,
                    'action' => $action])->all();
        $r = false;
        if (sizeof($puede) > 0) {
...
//$dataProvider->query->andFilterWhere(['event_id' => $id,]);
    ...
    /**
     * Lists all EstadosRepublica models.
     * @return mixed
     */
    public function actionIndex() {
        $this->canIndex('index', $nivel);
        $searchModel = new EstadosRepublicaSearch();
    ...
    /**
     * Displays a single EstadosRepublica model.
     * @param integer $id
     * @return mixed
    ...
    /**
     * Creates a new EstadosRepublica model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreate() {
        $this->can('create');
        $model = new EstadosRepublica();
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save()) {
                return $this->redirect(['view', 'id' => $model->esrep_id]);
    ...
    /**
     * crea en batch nuevosEstadosRepublica modelos hijos de una table.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionAddBatch($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->save()) {
                return $this->redirect(['EstadosRepublica/add', 'regis' => $ri]);
            } else {
    ...
    public function actionAdd($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save())
                return $this->redirect(['EstadosRepublica/view', 'id' => $model->perca_id]);
        }
        else {
    ...
    /**
     * Updates an existing EstadosRepublica model.
    ...
    /**
     * Deletes an existing EstadosRepublica model.
    ...
    /**
     * Finds the EstadosRepublica model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return EstadosRepublica the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id) {
        if (($model = EstadosRepublica::findOne($id)) !== null) {
            return $model;
        }

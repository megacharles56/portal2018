...
use app\models\VpermisosSearch;
//use app\models\Vpermisos;
...
/**
 * VpermisosController implements the CRUD actions for Vpermisos model.
 */
class VpermisosController extends Controller {
    ...
    public function ifcanIndex($action, &$nivel, $model = null) {
        ...
        $puede = Vpermisos::findBySql(
                        ...
                        . ' and ( permi_metodo = :action or permi_metodo = "*")', ['clase' => 'Vpermisos',
                    'usr' => $usr,
                    'action' => $action])->all();
        $r = false;
        if (sizeof($puede) > 0) {
...
//$dataProvider->query->andFilterWhere(['event_id' => $id,]);
    ...
    /**
     * Lists all Vpermisos models.
     * @return mixed
     */
    public function actionIndex() {
        $this->canIndex('index', $nivel);
        $searchModel = new VPermisosSearch();
    ...
    /**
     * Displays a single Vpermisos model.
     * @param integer $id
     * @return mixed
    ...
    public function actionCreate() {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save()) {
                return $this->redirect(['view', 'id' => $model->permi_id]);
    ...
    /**
     * crea en batch nuevosVpermisos modelos hijos de una table.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionAddBatch($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->save()) {
                return $this->redirect(['Vpermisos/add', 'regis' => $ri]);
            } else {
    ...
    public function actionAdd($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save())
                return $this->redirect(['Vpermisos/view', 'id' => $model->perca_id]);
        }
        else {
    ...
    /**
     * Updates an existing Vpermisos model.
    ...
    /**
     * Deletes an existing Vpermisos model.
    ...
    /**
     * Finds the Vpermisos model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Vpermisos the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id) {
        if (($model = Vpermisos::findOne($id)) !== null) {
            return $model;
        }

...
use Yii;
use app\models\TiposAsentamiento;
use app\models\TiposAsentamientoSearch;
...
/**
 * TiposAsentamientoController implements the CRUD actions for TiposAsentamiento model.
 */
class TiposAsentamientoController extends Controller {
    ...
    public function ifcanIndex($action, &$nivel, $model = null) {
        ...
        $puede = Vpermisos::findBySql(
                        ...
                        . ' and ( permi_metodo = :action or permi_metodo = "*")', ['clase' => 'TiposAsentamiento',
                    'usr' => $usr,
                    'action' => $action])->all();
        $r = false;
        if (sizeof($puede) > 0) {
...
//$dataProvider->query->andFilterWhere(['event_id' => $id,]);
    ...
    /**
     * Lists all TiposAsentamiento models.
     * @return mixed
     */
    public function actionIndex() {
        $this->canIndex('index', $nivel);
        $searchModel = new TiposAsentamientoSearch();
    ...
    /**
     * Displays a single TiposAsentamiento model.
     * @param integer $id
     * @return mixed
    ...
    /**
     * Creates a new TiposAsentamiento model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreate() {
        $this->can('create');
        $model = new TiposAsentamiento();
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save()) {
                return $this->redirect(['view', 'id' => $model->tasen_id]);
    ...
    /**
     * crea en batch nuevosTiposAsentamiento modelos hijos de una table.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionAddBatch($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->save()) {
                return $this->redirect(['TiposAsentamiento/add', 'regis' => $ri]);
            } else {
    ...
    public function actionAdd($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save())
                return $this->redirect(['TiposAsentamiento/view', 'id' => $model->perca_id]);
        }
        else {
    ...
    /**
     * Updates an existing TiposAsentamiento model.
    ...
    /**
     * Deletes an existing TiposAsentamiento model.
    ...
    /**
     * Finds the TiposAsentamiento model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return TiposAsentamiento the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id) {
        if (($model = TiposAsentamiento::findOne($id)) !== null) {
            return $model;
        }
